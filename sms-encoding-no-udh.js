function calculateMessageLength(str) {
    ...
    if (isUCS2) {
        ...
        if (calculatedLength > 70) {
            ...
            let length = segments.reduce((acc, seg) => {
        ...
        if (calculatedLength > 160) {
            ...
            let length = segments.reduce((acc, seg) => {
                return acc + seg.split('').reduce((subAcc, char) => {
...
// Example usage:
const result = calculateMessageLength("Rumors say there will be free healthy smoothies at the Twilio booth 🥤🍓🍍");
console.log(result);
